...
* **Moderator Channel**: Specify the channel to which all blacklisted messages will be forwarded to. From there, the [Channel Moderators](https://github.com/taranvohra/RocketChatDocs/blob/master/setup-and-administer-rocket.chat/roles-in-rocket.chat#rocket.chat-user-roles) can either `Approve` or `Reject` the message. e.g `general`
