...
The **Message Audit Log** allows you to verify who run audits and when a conversation was inspected. The user must have an `auditor-log` [role](https://github.com/taranvohra/RocketChatDocs/blob/master/setup-and-administer-rocket.chat/roles-in-rocket.chat) and `Can Audit Log` [permission](workspace-administration/permissions.md) to access the **Audit Logs**.
